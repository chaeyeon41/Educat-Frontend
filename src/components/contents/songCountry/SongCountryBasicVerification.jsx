...
import "./SongCountryBasicVerification.css";
import CheckBtn from "../../checkBtn/CheckBtn";
import axios from "axios";

const SongCountryBasicVerification = ({ contents, timer, handleNext }) => {
  ...
  return (
    <>
      <div className="basic-verification-songcountry-container">
        <div className="song-world-verification-image" onClick={toggleAudio}>
          <img src="/images/good.png" alt="동요이미지" />
        </div>
        <div className="basic-learning-wordabsorb-three">
          {contents.basic.countryThrees.map((item, index) => (
            <button
              key={index}
        ...
        <audio id="song-audio" controls style={{ display: "none" }}>
          <source
            src={contents.basic.countryTwoFiles[0].element}
            type="audio/mpeg"
          />
        </audio>
      </div>
...
export default SongCountryBasicVerification;
