...
const SongCountryDeepVerification = ({ contents, timer }) => {
    ...
    <>
      <div className="basic-verification-songcountry-container">
        <div className="song-world-verification-image" onClick={toggleAudio}>
          <img src="/images/good3.png" alt="동요이미지" />
        </div>
        <div className="basic-learning-wordabsorb-three">
          {contents.deep.countryThrees.map((item, index) => (
            <button
              key={index}
        ...
        <audio id="song-audio" controls style={{ display: "none" }}>
          <source
            src={contents.deep.countryTwoFiles[0].element}
            type="audio/mpeg"
          />
        </audio>
      </div>
...
export default SongCountryDeepVerification;
